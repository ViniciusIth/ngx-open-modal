...
export class ModalService {
  ...
  show(settings: ModalSettings) {
    const modalComponent =
      settings.rootContainer.createComponent(ModalComponent);

    modalComponent.instance.backdropDismiss = settings.dismissOnBackdrop;

    if (settings.input) {
      modalComponent.instance.inputData = settings.input;
    }
  ...
  async waitForDismiss(modalComponent: ComponentRef<ModalComponent>) {
    const data: ModalData = await lastValueFrom(
      modalComponent.instance.dismiss.pipe(first())
    );
...
export interface ModalData { [key: string]: any }
